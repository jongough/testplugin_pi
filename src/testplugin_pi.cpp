...
int testplugin_pi::Init(void) {
  ...
  m_testplugin_button_id = InsertPlugInToolSVG(
      _("Test Plugin"), m_ptpicons->m_s_testplugin_grey_pi,
      m_ptpicons->m_s_testplugin_pi, m_ptpicons->m_s_testplugin_toggled_pi,
      wxITEM_CHECK, _("Test Plugin"), wxS(""), NULL, testplugin_POSITION, 0,
      this);
#else
  m_testplugin_button_id = InsertPlugInTool(
      _("Test Plugin"), &m_ptpicons->m_bm_testplugin_grey_pi,
      &m_ptpicons->m_bm_testplugin_pi, wxITEM_CHECK, _("Test Plugin"), wxS(""),
      NULL, testplugin_POSITION, 0, this);
#endif
...
int testplugin_pi::GetPlugInVersionPatch() { return PLUGIN_VERSION_PATCH; }

int testplugin_pi::GetPlugInVersionPost() { return PLUGIN_VERSION_TWEAK; }

wxString testplugin_pi::GetCommonName() { return _T(PLUGIN_COMMON_NAME); }
...
int testplugin_pi::GetToolbarToolCount(void) { return 1; }
...
void testplugin_pi::OnToolbarToolDownCallback(int id) { return; }
...
void testplugin_pi::SaveConfig() {
  ...
  if (pConf) {
    ...
    if (m_bRecreateConfig) {
      ...
      pConf->Write(wxS("CloseSaveFileAferEachWrite"),
                   m_bCloseSaveFileAfterEachWrite);
      pConf->Write(wxS("AppendToSaveFile"), m_bAppendToSaveFile);
      pConf->Write(wxS("SaveIncommingJSONMessages"),
                   m_bSaveIncommingJSONMessages);
    }
  }
}
...
void testplugin_pi::LoadConfig() {
  ...
  if (pConf) {
    ...
    if (g_bSaveJSONOnStartup)
      m_tpControlDialogImpl->SetSaveJSONOnStartup(g_bSaveJSONOnStartup);
    wxString l_filepath;
    pConf->Read(wxS("JSONSaveFile"), &l_filepath, wxEmptyString);
    m_fnOutputJSON.Assign(l_filepath);
    if (m_fnOutputJSON != wxEmptyString)
      m_tpControlDialogImpl->SetJSONSaveFile(m_fnOutputJSON.GetFullPath());
    pConf->Read(wxS("JSONInputFile"), &l_filepath, wxEmptyString);
    m_fnInputJSON.Assign(l_filepath);
    if (m_fnInputJSON != wxEmptyString)
      m_tpControlDialogImpl->SetJSONInputFile(m_fnInputJSON.GetFullPath());
    pConf->Read(wxS("CloseSaveFileAferEachWrite"),
                &m_bCloseSaveFileAfterEachWrite, true);
    m_tpControlDialogImpl->SetCloseFileAfterEachWrite(
        m_bCloseSaveFileAfterEachWrite);
    pConf->Read(wxS("AppendToSaveFile"), &m_bAppendToSaveFile, true);
    m_tpControlDialogImpl->SetAppendToSaveFile(m_bAppendToSaveFile);
    pConf->Read(wxS("SaveIncommingJSONMessages"), &m_bSaveIncommingJSONMessages,
                false);
    m_tpControlDialogImpl->SetIncommingJSONMessages(
        m_bSaveIncommingJSONMessages);
  }
}
void testplugin_pi::GetODAPI() {
  ...
  if (g_ReceivedODAPIMessage != wxEmptyString &&
      g_ReceivedODAPIJSONMsg[wxT("MsgId")].AsString() == wxS("Version")) {
  ...
  SendPluginMessage(wxS("OCPN_DRAW_PI"), MsgString);
  if (g_ReceivedODAPIMessage != wxEmptyString &&
      g_ReceivedODAPIJSONMsg[wxT("MsgId")].AsString() ==
          wxS("GetAPIAddresses")) {
    m_iODAPIVersionMajor =
        g_ReceivedODAPIJSONMsg[_T("ODAPIVersionMajor")].AsInt();
    m_iODAPIVersionMinor =
        g_ReceivedODAPIJSONMsg[_T("ODAPIVersionMinor")].AsInt();
    ...
    sptr = g_ReceivedODAPIJSONMsg[_T("OD_FindClosestBoundaryLineCrossing")]
               .AsString();
    if (sptr != _T("null")) {
      sscanf(sptr.To8BitData().data(), "%p",
             &m_pODFindClosestBoundaryLineCrossing);
      m_bODFindClosestBoundaryLineCrossing = true;
    }
    sptr = g_ReceivedODAPIJSONMsg[_T("OD_FindFirstBoundaryLineCrossing")]
               .AsString();
    if (sptr != _T("null")) {
      sscanf(sptr.To8BitData().data(), "%p",
             &m_pODFindFirstBoundaryLineCrossing);
  ...
  wxString l_avail;
  wxString l_notavail;
  l_msg.Printf(_("OD Version: Major: %i, Minor: %i, Patch: %i, ODAPI Version: "
                 "Major: %i, Minor: %i\n"),
               m_iODVersionMajor, m_iODVersionMinor, m_iODVersionPatch,
               m_iODAPIVersionMajor, m_iODAPIVersionMinor);
  if (m_bOD_FindPointInAnyBoundary)
    l_avail.Append(_("OD_FindPointInAnyBoundary\n"));
  if (m_bODFindClosestBoundaryLineCrossing)
    l_avail.Append(_("OD_FindClosestBoundaryLineCrossing\n"));
  if (m_bODFindFirstBoundaryLineCrossing)
    l_avail.Append(_("OD_FindFirstBoundaryLineCrossing\n"));
  ...
  if (!m_bOD_FindPointInAnyBoundary)
    l_notavail.Append(_("OD_FindPointInAnyBoundary\n"));
  if (!m_bODFindClosestBoundaryLineCrossing)
    l_notavail.Append(_("OD_FindClosestBoundaryLineCrossing\n"));
  if (!m_bODFindFirstBoundaryLineCrossing)
    l_notavail.Append(_("OD_FindFirstBoundaryLineCrossing\n"));
  if (!m_bODCreateBoundary) l_notavail.Append(_("OD_CreateBoundary\n"));
  if (!m_bODCreateBoundaryPoint)
    l_notavail.Append(_("OD_CreateBoundaryPoint\n"));
...
bool testplugin_pi::ImportJSONFile() {
  ...
  if (!l_ffile.IsOpened()) {
    OCPNMessageBox_PlugIn(NULL, m_fnInputJSON.GetFullPath(),
                          _("File not found"), wxICON_EXCLAMATION | wxCANCEL);
    return false;
  }
  wxString l_str;
