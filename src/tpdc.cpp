...
void TPDC::SetVP(PlugIn_ViewPort *vp)
{
#ifdef USE_ANDROID_GLES2
...
void TPDC::DrawEndCap(float x1, float y1, float t1, float angle)
{
#ifndef USE_ANDROID_GLES2    
    const int steps = 16;
    float xa, ya;
...
void TPDC::DrawGLThickLine( float x1, float y1, float x2, float y2, wxPen pen, bool b_hiqual )
{
#ifdef ocpnUSE_GL
...
void TPDC::DrawLine( wxCoord x1, wxCoord y1, wxCoord x2, wxCoord y2, bool b_hiqual )
{
    if( dc ) {
        ...
        if( b_draw_thick ) {
            ...
            checkGlError("Before glUseProgram", "TPDC", __LINE__);
            GLint program = pi_colorv_tri_shader_program;
            glUseProgram(program);
            checkGlError("After glUseProgram", "TPDC", __LINE__);
...
void TPDC::DrawGLThickLines( int n, wxPoint points[],wxCoord xoffset, wxCoord yoffset, wxPen pen, bool b_hiqual )
{
#ifdef ocpnUSE_GL
    if(n < 2)
        return;
...
void TPDC::drawrrhelperGLES2( wxCoord x0, wxCoord y0, wxCoord r, int quadrant, int steps )
{
#ifdef ocpnUSE_GL
    float step = 1.0/steps, rs = 2.0*r*step, rss = rs*step, x, y, dx, dy, ddx, ddy;
    switch(quadrant) {
...
void TPDC::DrawDiskPattern( wxCoord x, wxCoord y, wxCoord innerRadius, wxCoord outerRadius, GLint textureID, wxSize textureSize )
{
    if( dc ) {
...
void TPDC::DrawPolygon( int n, wxPoint points[], wxCoord xoffset, wxCoord yoffset, float scale, float angle )
{
    if( dc )
        ...
        if(n > 4){
            ...
            checkGlError("Before glUseProgram", "TPDC", __LINE__);
            glUseProgram( program );
            checkGlError("Afore glUseProgram", "TPDC", __LINE__);
            checkGlError("glUseProgram", "TPDC", __LINE__);
            
            // Get pointers to the attributes in the program.
            GLint mPosAttrib = glGetAttribLocation( program, "aPos" );
            checkGlError("aPos", "TPDC", __LINE__);
            ...
            GLint bcolloc = glGetUniformLocation(program, "uColour");
            checkGlError("uColour", "TPDC", __LINE__);
            glUniform4fv(bcolloc, 1, bcolorv);
            if(bcolloc == -1)
                wxLogMessage(_("TPDC::DrawPolygon: bcolloc -1"));
            ...
            // Get pointers to the attributes in the program.
            GLint mPosAttrib = glGetAttribLocation( program, "aPos" );
            checkGlError("mPosAttrib", "TPDC", __LINE__);
            ...
            GLint matloc = glGetUniformLocation(program, "TransformMatrix");
            checkGlError("TransformMatrix", "TPDC", __LINE__);
            glUniformMatrix4fv( matloc, 1, GL_FALSE, (const GLfloat*)Q);
...
void TPDC::DrawPolygonPattern( int n, wxPoint points[], int textureID, wxSize textureSize, wxCoord xoffset, wxCoord yoffset, float scale, float angle )
{
    if( dc )
        dc->DrawPolygon( n, points, xoffset, yoffset );
#ifdef ocpnUSE_GL
        ...
        if(n > 3) {
           if(ConfigureBrush()) {       // Check for transparent brush
            ...
            checkGlError("mUvAttrib", "TPDC", __LINE__);

            // Rotate
            GLint uRotateMatrix  = glGetUniformLocation( program, "MVMatrix" );
            checkGlError("MVMatrix", "TPDC", __LINE__);
            ...
            checkGlError("texUni", "TPDC", __LINE__);
            ...
            glEnableVertexAttribArray( mPosAttrib );
            checkGlError("glEnableVertexAttribArray:mPosAttrib", "TPDC", __LINE__);
            glVertexAttribPointer( mPosAttrib, 2, GL_FLOAT, GL_FALSE, 2*sizeof(GLfloat), workBuf );
            checkGlError("glVertexAttribPointer:mPosAttrib", "TPDC", __LINE__);
            ...
            GLint bcolloc = glGetUniformLocation(program, "uColour");
            checkGlError("bcolloc", "TPDC", __LINE__);
            glUniform4fv(bcolloc, 1, bcolorv);
            checkGlError("glUniform4fv(bcolloc", "TPDC", __LINE__);
            ...
            if(n == 3){
                ...
                checkGlError("glEnableVertexAttribArray( mUvAttrib )", "TPDC", __LINE__);
                glVertexAttribPointer( mUvAttrib, 2, GL_FLOAT, GL_FALSE, 2*sizeof(GLfloat), UVCoords );
                checkGlError("glVertexAttribPointer", "TPDC", __LINE__);
                glDrawArrays(GL_TRIANGLES, 0, 3);
                checkGlError("glDrawArrays(GL_TRIANGLES)", "TPDC", __LINE__);
            }
            glUseProgram( 0 );
            checkGlError("glUseProgram(0)", "TPDC", __LINE__);
...
void APIENTRY TPDCcombineCallback( GLdouble coords[3], GLdouble *vertex_data[4], GLfloat weight[4],
        GLdouble **dataOut )
{
    GLvertex *vertex;
...
void APIENTRY TPDCvertexCallback( GLvoid* arg )
{
    GLvertex* vertex;
    vertex = (GLvertex*) arg;
...
void APIENTRY TPDCerrorCallback( GLenum errorCode )
{
   const GLubyte *estring;
   estring = gluErrorString(errorCode);
}

void APIENTRY TPDCbeginCallback( GLenum type )
{
    glBegin( type );
}

void APIENTRY TPDCendCallback()
{
    glEnd();
}
#endif
...
//    glUniformMatrix4fv( matloc, 1, GL_FALSE, (const GLfloat*)s_tessVP.vp_transform);
    ...
    checkGlError("glVertexAttribPointer( mUvAttrib)", "TPDC", __LINE__);
/*
    float colorv[4];
...
*/
        ...
        gluTessCallback( m_tobj, GLU_TESS_VERTEX, (_GLUfuncptr) &TPDCvertexCallback );
        gluTessCallback( m_tobj, GLU_TESS_BEGIN, (_GLUfuncptr) &TPDCbeginCallback );
        gluTessCallback( m_tobj, GLU_TESS_END, (_GLUfuncptr) &TPDCendCallback );
        gluTessCallback( m_tobj, GLU_TESS_COMBINE, (_GLUfuncptr) &TPDCcombineCallback );
        gluTessCallback( m_tobj, GLU_TESS_ERROR, (_GLUfuncptr) &TPDCerrorCallback );
...
void __CALL_CONVENTION TPDCPatterncombineCallback(GLdouble coords[3], GLdouble *vertex_data[4], GLfloat weight[4], GLdouble **dataOut)
{
    GLvertex *vertex;
...
void __CALL_CONVENTION TPDCPatternvertexCallback(GLvoid* arg)
{
    GLvertex* vertex;
    vertex = (GLvertex*) arg;
...
void __CALL_CONVENTION TPDCPatternerrorCallback(GLenum errorCode)
{
   const GLubyte *estring;
   estring = gluErrorString(errorCode);
}

void __CALL_CONVENTION TPDCPatternbeginCallback(GLenum type)
{
    glBegin( type );
}

void __CALL_CONVENTION TPDCPatternendCallback()
{
    glEnd();
}
#endif
...
void TPDC::DrawPolygonTessellatedPattern( int n, wxPoint points[], int textureID, wxSize textureSize, wxCoord xoffset, wxCoord yoffset )
{
    if( dc )
        dc->DrawPolygon( n, points, xoffset, yoffset );
#ifdef ocpnUSE_GL
...
//
        ...
        // Disable VBO's (vertex buffer objects) for attributes.
        glBindBuffer( GL_ARRAY_BUFFER, 0 );
        checkGlError("glBindBuffer", "TPDC", __LINE__);
        glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, 0 );
        checkGlError("glBindBuffer", "TPDC", __LINE__);
        ...
        checkGlError("glVertexAttribPointer", "TPDC", __LINE__);
        glEnableVertexAttribArray(mPosAttrib);
        checkGlError("glEnableVertexAttribArray", "TPDC", __LINE__);

        // Bind our texture to the texturing target.
        glBindTexture( GL_TEXTURE_2D, textureID );
        checkGlError("glBindTexture", "TPDC", __LINE__);
...
//        glUniform1f(yo, fmod(points[0].y, textureSize.y));
        ...
        bcolorv[3] = m_brush.GetColour().Alpha() / float(256);
        GLint bcolloc = glGetUniformLocation(program, "uColour");
        checkGlError("bcoloc", "TPDC", __LINE__);
        glUniform4fv(bcolloc, 1, bcolorv);
#endif
        // Tesselate
        m_tobj = gluNewTess();
        ...
        gluTessCallback( m_tobj, GLU_TESS_VERTEX, (_GLUfuncptr) &TPDCPatternvertexCallback );
        gluTessCallback( m_tobj, GLU_TESS_BEGIN, (_GLUfuncptr) &TPDCPatternbeginCallback );
        gluTessCallback( m_tobj, GLU_TESS_END, (_GLUfuncptr) &TPDCPatternendCallback );
        gluTessCallback( m_tobj, GLU_TESS_COMBINE, (_GLUfuncptr) &TPDCPatterncombineCallback );
        gluTessCallback( m_tobj, GLU_TESS_ERROR, (_GLUfuncptr) &TPDCPatternerrorCallback );
...
void TPDC::DrawPolygons(int n, int npoint[], wxPoint points[], wxCoord xoffset, wxCoord yoffset)
{
    if(g_textureId >= 0)
        DrawPolygonsPattern( n, npoint, points, g_textureId, wxSize(g_iTextureWidth, g_iTextureHeight), xoffset, yoffset);
}
        
void TPDC::DrawPolygonsPattern( int n, int npoint[], wxPoint points[], int textureID, wxSize textureSize, wxCoord xoffset, wxCoord yoffset, float scale, float angle)
{
#ifndef ANDROID
...
void TPDC::DrawTextEx( const wxString &text, wxCoord x, wxCoord y, float scaleFactor )
{
    if( dc )
...
void TPDC::SetTextureParms( GLint textureId, int width, int height )
{
...
void TPDC::DrawTexture( wxRect texRect, int width, int height, float scaleFactor, wxPoint position, float rotation, wxPoint rPivot)
{
        float w = width; 
        float h = height;
...
void TPDC::DrawTextureAlpha( wxRect texRect, int width, int height, float scaleFactor, wxPoint position, float rotation, wxPoint rPivot)
{
        float w = width; 
        float h = height;
